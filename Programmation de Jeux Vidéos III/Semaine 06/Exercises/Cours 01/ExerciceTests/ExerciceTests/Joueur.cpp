...
bool Joueur::validerAddresseCourriel(const string addresseCourriel) const
{
    ...
    string prefix = addresseCourriel.substr(0, addresseCourriel.find('@'));
    string sufix = addresseCourriel.substr(addresseCourriel.find('@')+1, addresseCourriel.find_last_of('.')-addresseCourriel.find('@')-1);
    string nomDomaine = addresseCourriel.substr(addresseCourriel.find_last_of('.') + 1, addresseCourriel.length());
	return true;
}
