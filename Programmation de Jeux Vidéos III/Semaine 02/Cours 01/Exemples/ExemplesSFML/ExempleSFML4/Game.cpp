...
void Game::getInputs()
{
  ...
  // lire à chaque frame et, du coup, de ne réagir que lorsque c'est vraiment important.
    ...
    {
      //Attention, il y a deux manière d'accéder aux inputs.  Avec les événements, nous allons vérifier le dernier événement survenu
      //Qui peut être un bouton appuyé, mais dès qu'un autre événement se produit, ce n'est plus le dernier événement survenu
      //On ne peut s'y fier pour une vérification continue de l'état d'un input
      //http://www.sfml-dev.org/tutorials/2.4/window-events-fr.php
      // On peut lire l'état d'un bouton avec
