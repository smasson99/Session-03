...
void printSortedArrayContainsElementV1(int numbers[], int size, int elementToFind)
{
    ...
    int min = 0;
    int pivot = (max - min) / 2;
    int currentNumber = numbers[pivot];
    bool finished = false;

    while (finished == false)
    {
        if (min >= max || currentNumber == elementToFind)
        {
            if (currentNumber == elementToFind)
            {
                cout << "Le nombre : " << elementToFind << " a bel et bien été trouvé en position : " << pivot << "." << endl;
            }
            else
            {
                cout << "Le nombre : " << elementToFind << " n'a pas été trouvé dans le tableau." << endl;
            }
            finished = true;
        }
        if (elementToFind > currentNumber)
        {
            min = pivot + 1;
        }
        else // elementToFind < currentNumber
        {
            max = pivot - 1;
        }
        pivot = (max - min) / 2;
...
{
    //Importer le système de caractères:
    setlocale(LC_ALL, "");
